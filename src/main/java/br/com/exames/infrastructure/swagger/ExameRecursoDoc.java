...
public interface ExameRecursoDoc {
	...
	@ApiOperation(value = "Salva o exame do cliente no prontuário", notes = "<!DOCTYPE html>" + "<html>" + "<head>"
			...
			+ "</tr>" + "<tr> <td>CRM</td>"
			+ "<td> Deve ser informado o CRM do medico que realizou o exame. </strong></td>" + "<td>Query</td>"
			+ "<td>🛑</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "<tr> <td>Data Exame</td>" + "<td> Deve ser a data em que o exame foi realizado. </strong></td>"
			+ "<td>Query</td>" + "<td>🛑</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "<tr> <td> Indicacao</td>"
			+ "<td> Deve ser informado o motivo da indicação do exame ao paciente. </strong></td>" + "<td>Query</td>"
			+ "<td>🛑</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "<tr> <td> Parte-Corpo</td>"
			+ "<td> Deve ser informado  em qual parte do corpo o paciente foi submetido ao exame.</strong></td>"
			+ "<td>Query</td>" + "<td>🛑</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "<tr> <td> Usuario</td>"
			+ "<td> Deve ser informado o nome do usuário que está enviando essas informações ao prontuário. </strong></td>"
			+ "<td>Query</td>" + "<td>🛑</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "</body>" + "</html>")
	@ApiResponse(code = 200, message = "Retorno obtido ao executar corretamente o recurso.")
	@ResponseStatus(HttpStatus.OK)
	public ResponseEntity<Optional<ResponseObject>> uploadFile(
			@ApiParam(value = "File to be uploaded", required = true) @RequestPart("file") MultipartFile file,
			@RequestParam(required = false) @DateTimeFormat(pattern = "yyyy-MM-dd") String dataRealizacao,
			@RequestParam(required = false) String nmUsuario, @RequestParam(required = true) String cpfPaciente,
			@RequestParam(required = false) String crmMedico, @RequestParam(required = false) String parteCorpo,
			@RequestParam(required = false) String indicacao);
	...
	@ApiOperation(value = "Listagem de exames no servidor", notes = "<!DOCTYPE html>" + "<html>" + "<head>"
			+ "<title>upload.md</title>" + "<meta http-equiv=\"Content-type\" content=\"text/html;charset=UTF-8\">"
			+ "<style>"
			+ "/* https://github.com/microsoft/vscode/blob/master/extensions/markdown-language-features/media/markdown.css */"
			+ "/*---------------------------------------------------------------------------------------------"
			...
			+ "      theme: document.body.classList.contains('vscode-dark') || document.body.classList.contains('vscode-high-contrast')"
			+ "          ? 'dark'" + "          : 'default'" + "    });" + "  </script>"
			+ "<h2 id=\"upload\">Busca todos os exames do servidor</h2>" + "<hr>"
			+ "<p>Este recurso realiza uma consulta em todos os arquivos que estão salvos no servidor</p>"
			+ "</tr>" + "</body>" + "</html>")
	@ApiResponse(code = 200, message = "Retorno obtido ao executar corretamente o recurso.")
	@ResponseStatus(HttpStatus.OK)
	...
	@ApiOperation(value = "Faz o download de um exame ", notes = "<!DOCTYPE html>" + "<html>" + "<head>"
			+ "<title>upload.md</title>" + "<meta http-equiv=\"Content-type\" content=\"text/html;charset=UTF-8\">"
			+ "<style>"
			+ "/* https://github.com/microsoft/vscode/blob/master/extensions/markdown-language-features/media/markdown.css */"
			+ "/*---------------------------------------------------------------------------------------------"
			...
			+ "      theme: document.body.classList.contains('vscode-dark') || document.body.classList.contains('vscode-high-contrast')"
			+ "          ? 'dark'" + "          : 'default'" + "    });" + "  </script>"
			+ "<h2 id=\"upload\">Busca o exame pelo nome unico gerado na base da dados</h2>" + "<hr>"
			+ "<p>Este recurso faz download de um exame de acordo com o nome unico que foi gerado e salvo no prontuário do paciente"
			+ " que deve ser informado no parâmetro de requisição.</p>"
			+ "<p></p>"
			+ "<h3 id=\"par%C3%A2metros-de-requisi%C3%A7%C3%A3o\">Parâmetros de requisição</h3>" + "<table>" + "<thead>"
			+ "<tr>" + "<th>Parâmetro</th>" + "<th>Descrição</th>" + "<th>Tipo</th>" + "<th>Obrigatório?</th>" + "</tr>"
			+ "</thead>" + "<tbody>" + "<tr>" + "<td>File</td>"
			+ "<td> Deve ser informado o nome gerado para o arquivo seguido da extensão, ex: 00011-NOME.PDF  .</strong></td>" + "<td>Path</td>" + "<td>✅</td>" + "</tr>" + "<tr>"
			+ "</tr>" + "</body>" + "</html>")
	@ApiResponse(code = 200, message = "Retorno obtido ao executar corretamente o recurso.")
	@ResponseStatus(HttpStatus.OK)
